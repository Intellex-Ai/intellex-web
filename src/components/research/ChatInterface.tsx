...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
    ...
    const renderMessages = useMemo(() => {
        // Guard against duplicate message ids so React keys stay unique
        const occurrences = new Map<string, number>();

        return messages.map((msg, index) => {
            const count = occurrences.get(msg.id) ?? 0;
            occurrences.set(msg.id, count + 1);

            return {
                msg,
                renderKey: count === 0 ? msg.id : `${msg.id}-${count}-${msg.timestamp ?? index}`,
            };
        });
    }, [messages]);
    ...
    return (
        <div className="flex flex-col h-full bg-black relative overflow-hidden">
            ...
            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-8 custom-scrollbar relative z-10">
                {messages.length === 0 && (
                ...
                <AnimatePresence initial={false}>
                    {renderMessages.map(({ msg, renderKey }) => (
                        <motion.div
                            key={renderKey}
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.4, ease: "easeOut" }}
                            className={`flex gap-4 md:gap-6 ${msg.senderType === 'user' ? 'flex-row-reverse' : ''}`}
                        >
                            {/* Avatar */}
                            <div className={`
                ...
                <AnimatePresence>
                    {isLoading && (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1 h-10 px-4 bg-primary/5 border border-primary/10 rounded-sm">
                                <motion.div
                                ...
                                <motion.div
                                    ...
                                    className="w-1.5 h-1.5 bg-primary rounded-full"
                                />
                                <span className="ml-2 text-[10px] font-mono text-primary uppercase tracking-widest animate-pulse">Processing</span>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
            ...
            {/* Input Area */}
            <div className="absolute bottom-0 left-0 right-2 p-4 md:p-6 bg-gradient-to-t from-black via-black/90 to-transparent z-20">
