...
export default function Hero() {
    ...
    const fadeIn = isLowEnd 
        ? {} 
        : { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 } };
    
    const scaleIn = isLowEnd
        ? {}
    ...
    return (
        <section className="relative min-h-screen flex items-center justify-center px-8 overflow-hidden md:px-4">
            ...
            <div className="max-w-[1400px] w-full text-center flex flex-col items-center relative z-10 pt-20">
                <motion.div
                    {...fadeIn}
                    transition={isLowEnd ? undefined : { duration: 0.6, ease: [0.23, 1, 0.32, 1] }}
                    className="mb-8"
                >
                    <Badge variant="neutral" className={isLowEnd ? "bg-surface/80 border-white/20" : "bg-surface/50 backdrop-blur border-white/20"}>
                ...
                <motion.h1
                    {...scaleIn}
                    transition={isLowEnd ? undefined : { duration: 0.8, delay: 0.2, ease: [0.23, 1, 0.32, 1] }}
                ...
                >
                    <span className={`block text-transparent [-webkit-text-stroke:1px_rgba(255,255,255,1)] md:[-webkit-text-stroke:2px_rgba(255,255,255,1)] mb-2 md:mb-4 ${isLowEnd ? '' : 'drop-shadow-[0_0_20px_rgba(255,255,255,0.8)]'}`}>
                        <TextScramble text="INTELLIGENCE" duration={1500} />
                    </span>
                    <span className={`text-white ${isLowEnd ? '' : 'drop-shadow-[0_0_40px_rgba(255,77,0,0.8)]'} text-shadow-lg`}>
                ...
                <motion.p
                    {...fadeIn}
                    transition={isLowEnd ? undefined : { duration: 0.6, delay: 0.4, ease: [0.23, 1, 0.32, 1] }}
                ...
                <motion.div
                    {...fadeIn}
                    transition={isLowEnd ? undefined : { duration: 0.5, delay: 0.4 }}
